#!/usr/bin/env python3
"""Emit pinned Marshal numeric constants into a Lean witness comment block.

Reads docs/generated/*.json and verifies sync with MarshalCertAdapter / MarshalCertLift.

Usage:
  python tools/Analysis/EmitMarshalLeanCert.py
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Emit/sync Marshal Lean pinned cert values")
    parser.add_argument("--check", action="store_true", help="Exit 1 if JSON drifts from pinned Lean")
    ...
    if args.check:
        ...
        if drift:
            print("Marshal pinned Lean cert drift detected:", file=sys.stderr)
        ...
        if WEDGE_JSON.is_file():
            ...
            if wedge_drift:
                print("Marshal wedge pinned Lean cert drift detected:", file=sys.stderr)
            ...
            if not wedge_live.get("accumulationGridOk"):
                ...
                return 1
        print("Marshal pinned Lean cert matches JSON.")
        return 0

    print("-- Marshal pinned cert (sync with MarshalCertAdapter / MarshalCertLift / MarshalZeroAsymptotics)")
